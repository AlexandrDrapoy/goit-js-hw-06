...
//   скільки вказано в amount і додає їх у div#box розміри найпершого <div> - 30px на 30px.
...
function createBoxes(amount) {
  ...
  for (let i = 0; i < amount; i += 1) {
    ...
    markup += `<div style="width:${size + i * 10}px; height:${
      size + i * 10
    }px; background-color: ${currentColor}; display:block;"> </div>`;
  }
...
document.querySelector("[data-create]").addEventListener("click", () => {
  const amount = Number(
    document.querySelector("#controls").firstElementChild.value
  );
  createBoxes(amount);
});
